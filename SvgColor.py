...
class SvgColor:
    ...
    def colorSvg2Vd(cls, svgColorValue: str):
        ...
        if color.startswith('rgb(') and color.endswith(')'):
            ...
            builder = '#'
            for i in range(3):
                component = self.getColorComponent(numbers[i].strip(), svgColorValue)
        ...
        if color.startswith("rgba(") and color.endswith(')'):
            ...
            builder = '#'
            for i in range(4):
                component = self.getColorComponent(numbers[(i + 3) % 4].strip(), svgColorValue)
                builder += '%02X' % component
            assert len(builder) == 9
            return builder
        return self.colorMap.get(color.lower())
    ...
    def getColorComponent(cls, colorComponent: str, svgColorValue: str):
        try:
            if colorComponent.endswith('%'):
                ...
                return self.clampColor(math.round(value * 255.0 / 100.0))
            return self.clampColor(int(colorComponent))
